...
date: "DRAFT version currently under revision, finalised 2025/6 version due for release in September 2025. Last updated on `r format(Sys.time(), '%d %B, %Y')`"
...
# can read style file from http rather than storing locally
# csl: /Users/duncan/Zotero/styles/harvard-imperial-college-london.csl
...
<!--Add note for Word ouput at top of document
e.g. warning, this version is for editors only who prefer not to use github
turn on track changes etc see 
https://github.com/dullhunk/cdyf/issues/1194
-->

# Welcome to Industrial Experience (IE) {-}

Welcome to the Industrial Experience (IE) placement handbook. This student handbook is for undergraduates studying Computer Science in the School of Engineering at the University of Manchester. The handbook has been written to help you (and your personal tutor) before, during and after your year-long placement in industry and is [also available as a pdf](https://placement-handbook.cs.manchester.ac.uk/placement-handbook.pdf). test.
...
1. **Duncan Hull**: Employability lead and employability tutor, Computer Science [manchester.ac.uk/staff/duncan.hull/contact](https://personalpages.manchester.ac.uk/staff/duncan.hull/contact), see figure \@ref(fig:team-fig)
...
## Industrial Experience in dark mode {#darkmode}
We often get asked if teaching materials are available in [dark mode](https://en.wikipedia.org/wiki/Light-on-dark_color_scheme). The good news is that unlike some other course materials, this handbook **is** available in dark mode. If you're reading this in a web browser, you can choose whatever mode you prefer, see figure \@ref(fig:darkmode-fig).

```{r darkmode-fig, echo = FALSE, fig.align = "center", fig.cap = "(ref:captiondarkmode)", out.width = "100%"}
knitr::include_graphics("images/darkmode.png")
```

(ref:captiondarkmode) If you'd prefer to read this handbook in a [light on dark colour scheme](https://en.wikipedia.org/wiki/Light-on-dark_color_scheme), move your mouse to the `A` at the top of the screen and select `Night` as shown circled in red here. Dark mode works in the web browser only, not the [pdf](https://placement-handbook.cs.manchester.ac.uk/placement-handbook.pdf) or [word](https://placement-handbook.cs.manchester.ac.uk/placement-handbook.docx) version.
...
* Edit the source in markdown. That's all the `*.Rmd` files [available on github](https://github.com/search?q=repo%3AUoMCS%2Fplacement-handbook++language%3ARMarkdown&type=code) (github login required), which are used to generate all the outputs including: 
    ...
    + the [placement-handbook.cs.manchester.ac.uk](https://placement-handbook.cs.manchester.ac.uk), all the `*.html` files you see if you are reading this text in a web browser
* Clone or fork the repository [github.com/uomcs/placement-handbook](https://github.com/uomcs/placement-handbook), make changes and submit a pull request 
...
```
(ref:captionteam) Hello and welcome from Duncan Hull (left) employability lead and employability tutor for the Department of Computer Science and David Petrescu (right) Placement year tutor for your Industrial Experience, our contact details are shown in section \@ref(team)
